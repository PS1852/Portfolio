    ...
    <section ref={sectionRef} id="experience" className="py-28 md:py-36 bg-white relative overflow-hidden">
      ...
      <div className="section-padding max-w-5xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="experience-cards relative flex flex-col gap-16">
            {EXPERIENCES.map((exp, idx) => (
              <div
                key={exp.id}
                className={`experience-card relative flex items-center gap-8 ${
              ...
              >
                ...
                <div className={`flex-1 ${idx % 2 === 0 ? 'text-right' : 'text-left'}`}>
                  <div className="glass-card p-8 inline-block w-full max-w-md hover:shadow-lg transition-shadow duration-300 group">
                    {/* Company badge */}
                    <div className={`flex items-center gap-3 mb-4 ${idx % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
                      <span className="tag bg-emerald-50 text-emerald-700 border border-emerald-200 font-mono">
                        {exp.dates}
                      </span>
                    </div>

                    <h3
                      className="text-xl font-bold text-ink mb-1 group-hover:text-emerald-700 transition-colors"
                      style={{ fontFamily: 'Playfair Display, serif' }}
                    >
                      {exp.role}
                    </h3>

                    <p className="text-emerald-600 font-medium text-sm mb-4 font-mono">
                      @ {exp.company}
                    </p>
