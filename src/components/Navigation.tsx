    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled
            ? 'bg-[#1a0a2e]/90 backdrop-blur-xl border-b border-white/10'
            : 'bg-transparent'
          }`}
      >
      ...
      >
        ...
        />
        <div
          className={`absolute top-16 left-0 right-0 bg-[#1a0a2e] border-b border-white/10 transition-transform duration-500 ${isMobileMenuOpen ? 'translate-y-0' : '-translate-y-full'
            }`}
        >
          <div className="px-4 py-6 space-y-4">
            {navLinks.map((link) => (
              <button
                key={link.id}
                onClick={() => scrollToSection(link.id)}
                className="block w-full text-left text-white text-lg font-medium py-2 hover:text-purple-400 transition-colors"
              >
                {link.label}
              </button>
            ))}
